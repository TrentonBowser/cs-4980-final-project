...
There are a total of `r lenth(unique(cleaned_crime_data$IUCR))` unique crimes
...
There are a total of `r length(unique(cleaned_art_data$``PARK NUMBER``)` unique locations for art in the dataset. 
...
#### Visualization
We wanted to visualize some of the relationships between our variables of interest. Here we have the locations of the crimes mapped with wether or not they resulted in an arrest.
```{r}
...
We also looked at the relationship between type of crime and wether it resulted in an arrest.
```{r}
combined_data %>% mutate(fac_type=`Primary Type` %>% fct_infreq()) %>%
  mutate(fac_type=fct_lump(fac_type, n=10)) %>%
...
# Model accuracy
cat('Logistic Regression Model Accuracy w/ just crime location: ', round(mean(predicted.classes == test.data$Arrest),4), '%')
```
...
predicted.classes <- ifelse(probabilities > 0.5, "TRUE", "FALSE")
# Model accuracy
cat('Logistic Regression Model Accuracy w/o added distances: ', round(mean(predicted.classes == test.data$Arrest),4), '%')
```
...
predicted.classes <- ifelse(probabilities > 0.5, "TRUE", "FALSE")
# Model accuracy
cat('Logistic Regression Model Accuracy w/ added distances: ', round(mean(predicted.classes == test.data$Arrest),4), '%')
```
